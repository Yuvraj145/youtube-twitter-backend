...
const registerUser = asyncHandler(async (req, res) => {
  ...
  //validation kuch mistake nahi kiya hai like empty and wrong formate
  if (
  ...
  //check already exists or not so we can check by email or username
  ...
  //if files then upload then upload to cloudinary ,cross check for avatar
  const avatar = await uploadOnCloudinary(avatarLocalPath);
  console.log("avatar hai kya bhai", avatar);
  const coverImage = await uploadOnCloudinary(coverImageLocalPath);
  if (!avatar) {
    throw new ApiError(400, "Avatar file is required");
  }
  //create user object - create or save data in db
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  //check username and email given
  if (!email && !username) {
    throw new ApiError(401, "Username or Email is required");
  }
  //find the user then validate the data
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  const incomiingRefreshToken =
    req.cookies.refreshToken || req.body.refreshToken;
  if (!incomiingRefreshToken) {
    throw new ApiError(401, "Unauthorized request");
  }

  try {
    const decodedToken = jwt.verify(
      incomiingRefreshToken,
    ...
    if (incomiingRefreshToken !== user?.refreshToken) {
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  // const usre = await User.find({username})
  ...
  console.log("channel ", channel); //what data type does aggregate returns
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          ...
          //look up done all data in owner fileds
          {
            $addFields: {
              owner: {
                $first: "$owner",
